Raphael.fn.g.piechart=function(e,d,o,b,l,radius,offset,ieMult,leftOffset, textAlign){
	l=l||{};
	radius=radius||5;
	offset=offset||5;
	ieMult=ieMult||1;
	leftOffset=leftOffset||0;
	var k=this,m=[],g=this.set(),n=this.set(),j=this.set(),u=[],w=b.length,x=0,A=0,z=0,c=9,y=true;n.covers=g;
	if(w==1){  //****************************************************************************Simply draws the circle of a chart with only 1 value
		j.push(this.circle(e,d,o).attr({fill:this.g.colors[0],stroke:l.stroke||"#fff","stroke-width":l.strokewidth==null?1:l.strokewidth}));
		l.href&&l.href[0]&&j.attr({href:l.href});
		l.attr=function(){};
		//g.push(this.circle(e,d,o).attr(this.g.shim));
		g.push(j);
		A=b[0];b[0]={value:b[0],order:0,valueOf:function(){
			return this.value;
		}};
		j[0].middle={x:e,y:d};
		j[0].mangle=180;  //***************************************************************************End draw circle
	} else { 
		function t(F,E,i,H,D,M){
			var J=Math.PI/180;
			B=F+i*Math.cos(-H*J);
			p=F+i*Math.cos(-D*J);
			G=F+i/2*Math.cos(-(H+(D-H)/2)*J);
			L=E+i*Math.sin(-H*J);
			K=E+i*Math.sin(-D*J);
			C=E+i/2*Math.sin(-(H+(D-H)/2)*J);
			I=["M",F,E,"L",B,L,"A",i,i,0,+(Math.abs(D-H)>180),1,p,K,"z"];
			I.middle={x:G,y:C};
			return I;
		}
		for(var v=0;v<w;v++){
			A+=b[v];
			b[v]={  //************** Skipped in Ruby
				value:b[v],order:v,valueOf:function(){
					return this.value;
				}
			};  //************** End skipped
		}
		//b.sort(function(p,i){  //*********************** Skipped in Ruby
		//	return i.value-p.value;
		//});    //************************ End skipped
		w=Math.min(c+1,b.length);    //Skipped
		z&&b.splice(w)&&(b[c].others=true);  //Skipped
		for(var v=0;v<w;v++){
			if (A === 0) A = 906;
			var f=x-360*b[v]/A/2;
			if(!v){
				x=90-f;f=x-360*b[v]/A/2;
			}
			if(l.init){
		...
		for(var v=0;v<w;v++){  //Makes slices clickable??
			var q=k.path(m[v].attr("path")).attr(this.g.shim);
	...
	var a=function(G,B,r,p){   //*************************************************** Creates labels
		var K=e+o+o/5,J=d,F=J+10;G=G||[];
		...
		for(var E=0;E<w;E++){
			...
			n.labels.push(k.set());
			n.labels[E].push(k.g[r](K+offset,F,radius).attr({fill:L,stroke:"none"}));
			...
			if (navigator.userAgent.search(/MSIE/) === -1){
				...
				F+=D.getBBox().height*ieMult;
				//F+=13*ieMult;
			}
		}
		var H=n.labels.getBBox(),I={east:[0,-H.height/2],west:[-H.width-2*o-20,-H.height/2],north:[-o-H.width/2,-o-H.height-10],south:[-o-H.width/2,o+10],southwest:[-o-H.width/2-leftOffset,o+45]}[p];
		//var I = {southwest:[-o-textAlign/2-leftOffset, o+45]}[p];
